  ...
  function renderMyPlaylists(playlists) {
    ...
    playlists.forEach(playlist => {
      ...
      btn.addEventListener('click', () => {
        // Map stored track fields to the shape showPlaylistView expects
        ...
        // Switch to albums view (where #playlistView lives), then open it
        if (window.showView) window.showView('albums');
        if (typeof window.showPlaylistView === 'function') {
          window.showPlaylistView(playlist.name, mappedTracks, playlist.id);
        }
      });
