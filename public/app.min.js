...
function doSearch(q, options = {}){
    ...
    async function openTracksModal(album, sourceCard=null){
      ...
      validTracks.forEach((t, idx) => {
        ...
        // Mad Mixer (dark unless MAD_MIXER_ENABLED): opens the track in the stem splitter.
        // Passes the catalogue record id only — the server resolves the audio.
        if (window.__MAD_MIXER && t.recordId) {
          const btnMix = document.createElement('button');
          btnMix.type = 'button';
          btnMix.className = 'btn small btn-mad-mixer';
          btnMix.title = 'Split this track into stems in Mad Mixer';
          btnMix.innerHTML = '🎚 Mad Mixer';
          btnMix.addEventListener('click', () => window.open(`/mixer?t=${encodeURIComponent(t.recordId)}`, '_blank', 'noopener'));
