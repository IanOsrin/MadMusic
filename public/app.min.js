...
function hideLanding(){ /* no-op: placeholder removed */ }function doSearch(q, options = {}){
    ...
    function handlePlay(btn, row, src){
      ...
      // Delegate audio playback to _PLAYER (single source of truth for all views).
      // For resuming the same track player.src is already correct — just resume.
      // For a new track _PLAYER.playTrack() sets src, updates the mini-player,
      // and returns the play() Promise so we can chain our UI updates here.
      let playPromise;
      if (resumingSameTrack) {
        playPromise = player.play();
      } else {
        const m = (row && row._meta) || {};
        let pic = m.picture || '';
        if (pic && !/^https?:/.test(pic) && !pic.startsWith('/api/')) {
          pic = '/api/container?u=' + encodeURIComponent(pic);
        }
        if (window._PLAYER && window._PLAYER.playTrack) {
          playPromise = window._PLAYER.playTrack(src, {
            title:  m.trackName  || '',
            artist: m.trackArtist || m.albumArtist || '',
            artUrl: pic
          });
        } else {
          // _PLAYER not ready yet — direct fallback
          player.src = src;
          playPromise = player.play();
        }
      }
