    ...
    function handlePlay(btn, row, src){
      ...
      player.play()
        .then(() => {
          ...
          // Sync track state with _PLAYER so the unified mini-player controls
          // (toggle, progress, volume) stay in sync with app.min.js playback.
          if (window._PLAYER && window._PLAYER.syncTrack) {
            const m   = (row && row._meta) || {};
            let   pic = m.picture || '';
            if (pic && !/^https?:/.test(pic) && !pic.startsWith('/api/')) {
              pic = '/api/container?u=' + encodeURIComponent(pic);
            }
            window._PLAYER.syncTrack(src, {
              title:  m.trackName  || '',
