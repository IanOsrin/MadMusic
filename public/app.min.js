    ...
    async function sendStreamEvent(type, metaOverride, positionOverride, durationOverride, deltaOverride){
      ...
      const body = {
        ...
        // Guests only ever hear the server-clipped 30 s preview — flag it so
        // royalty reporting can filter PlaybackMode ≠ PREVIEW.
    ...
    function renderPlaylistsPanel(){
      ...
      if (playlistsList) {
        ...
        list.forEach((pl) => {
          // Row is a container, not a button, so the artwork control can sit
          // beside the select control without nesting one button in another.
          ...
          main.type = 'button';
          main.className = 'playlist-pill-main';

          const icon = document.createElement('span');
          icon.className = 'playlist-pill-icon';
          if (pl.artwork) {
            const img = document.createElement('img');
            img.src = playlistIconThumb(pl.artwork, 300);
            img.alt = '';
            img.loading = 'lazy';
            // Derivative may not exist yet; master is always there.
            img.onerror = () => {
              if (img.src !== pl.artwork) { img.src = pl.artwork; return; }
              icon.textContent = '♪';
            };
          ...
          info.className = 'playlist-pill-info';
          const nameEl = document.createElement('span');
          nameEl.className = 'playlist-pill-name';
          nameEl.textContent = pl.name || 'Untitled';
          const countEl = document.createElement('span');
          countEl.className = 'playlist-pill-count';
          ...
          editBtn.type = 'button';
          editBtn.className = 'playlist-pill-edit';
          editBtn.textContent = '✎';
          editBtn.title = `Change artwork for ${pl.name || 'this playlist'}`;
          editBtn.setAttribute('aria-label', editBtn.title);
          editBtn.addEventListener('click', async (e) => {
            e.stopPropagation();
            if (typeof window.massPickPlaylistIcon !== 'function') return;
            const chosen = await window.massPickPlaylistIcon(pl.artwork || '');
            if (chosen === null) return;                       // cancelled
            if ((pl.artwork || '') === chosen) return;         // unchanged
            ...
            try {
              ...
              const json = await res.json().catch(() => ({}));
              if (!res.ok || !json?.ok) throw new Error(json?.error || 'Failed to update artwork');
              pl.artwork = chosen;
              renderPlaylistsPanel();
            } catch (err) {
              window.alert(err?.message || 'Unable to update artwork');
              editBtn.disabled = false;
            }
          });

          pill.append(main, editBtn);
    ...
    function buildPlaylistTrackPayload(album, track, playableSrc){
      ...
      return {
        ...
        artwork: album?.picture || album?.artwork || album?.artworkUrl || '',
    ...
    async function refreshCurrentUser(){
      // Derive user identity from access token email (set by token overlay)
    ...
    const normalizeSearchFilters = (filters = {}) => ({
      ...
      artist: typeof filters.artist === 'string' ? filters.artist.trim() : ''
    });
    ...
    function resolvePlayableSrc(rawSrc){
      ...
      if (/^https?:\/\/s3[.-]/.test(src)) return src;
      // the media CDN fronting the bucket is equally direct-safe
      if (window.__MEDIA_CDN && src.includes('//' + window.__MEDIA_CDN + '/')) return src;
    ...
    async function probeAudioSource(src, attempt = 0){
      ...
      const isDirectS3 = /^https?:\/\/.*\.s3[.-]/i.test(key) || /^https?:\/\/s3[.-]/i.test(key)
        || !!(window.__MEDIA_CDN && key.includes('//' + window.__MEDIA_CDN + '/'));
      if (isDirectS3) {
      ...
      try {
        // Check if it's an S3/CDN URL - if so, don't send credentials (CORS issue)
        const isS3 = /^https?:\/\/.*\.s3[.-]/.test(src) || /^https?:\/\/s3[.-]/.test(src)
          || !!(window.__MEDIA_CDN && src.includes('//' + window.__MEDIA_CDN + '/'));
      ...
      for (const song of songs) {
        ...
        try {
          ...
          wrap.addEventListener('click', () => {
            ...
            run(albumArtist, { artist: albumArtist });
          });
...
function doSearch(q, options = {}){
      abortInFlight();
      const ctrl = new AbortController();
      inFlight = ctrl;
      const params = new URLSearchParams();
      const artistValue = typeof options.artist === 'string' ? options.artist.trim() : '';
      const genreValue = typeof options.genre === 'string' ? options.genre.trim() : '';
      if (artistValue) {
        // Explicit artist search — pass as ?artist= so backend restricts to artist fields only
    ...
    function groupAlbums(items){
      ...
      for (const rec of items) {
        ...
        const composersRaw = normTitle(f['Composers'] || '');
        const comps = composersRaw
          ? composersRaw
          : composersFrom(f).map(value => normTitle(value || '')).join(', ');
        const prod  = normTitle(f['Producers'] || f['Producer'] || '');
        ...
        const downloadPrice = parseFloat(f['Download_Price'] || f['DownloadPrice'] || 0) || 0;
        ...
        const aiBpm    = String(f['AI_BPM']    || f['AI BPM']    || '').trim();
        const aiKey    = String(f['AI_Key']    || f['AI Key']    || '').trim();
        const aiMood   = String(f['AI_Mood']   || f['AI Mood']   || '').trim();
        const aiEnergy = String(f['AI_Energy'] || f['AI Energy'] || '').trim();
        const leadSilence = parseFloat(f['AI_LeadSilence']) || 0;
    ...
    async function loadPublicPlaylistTracks(name){
      ...
      try {
        ...
        const sortedTracks = tracks.slice(); // preserve FileMaker record order
    ...
    function renderPublicPlaylistView(){
      ...
      const bioBox = document.getElementById('publicPlaylistBio');
      ...
      if (!hasActive) {
        ...
        if (bioBox) { bioBox.hidden = true; bioBox.innerHTML = ''; bioBox.dataset.artist = ''; }
        if (albumsEl) albumsEl.style.display = '';
        return;
      }
      ...
      // matches (loadArtistBio hides the box on no match). Guard on the derived
      // artist so repeated renders (loading → loaded) don't re-fetch/flicker.
      if (bioBox) {
        const bioArtist = (typeof window.artistFromMadAbout === 'function')
          ? window.artistFromMadAbout(activePublicPlaylist) : '';
        if (!bioArtist) {
          bioBox.hidden = true; bioBox.innerHTML = ''; bioBox.dataset.artist = '';
        } else if (bioBox.dataset.artist !== bioArtist) {
          bioBox.dataset.artist = bioArtist;
          if (typeof window.loadArtistBio === 'function') window.loadArtistBio(bioArtist, 'publicPlaylistBio');
        }
      }
    ...
    async function openTracksModal(album, sourceCard=null){
            ...
            if (data.items && data.items.length > 0) {
              ...
              if (currentMode === 'search') {
                ...
                const transformedTracks = data.items.map(item => {
                  const f = item.fields || {};
                  const mp3Info = pickFieldValue(f, AUDIO_FIELD_CANDIDATES);
                  const _trim = (v) => (v !== undefined && v !== null) ? String(v).trim() : '';
                  const producer = _trim(f['Producers'] || f['Producer'] || '');
                  const composers = _trim(f['Composers'] || '')
                    || [f['Composer'], f['Composer 2'], f['Composer 3']].filter(Boolean).map(_trim).filter(Boolean).join(', ');
                  return {
                    name: f['Track Name'] || '',
                    mp3: mp3Info.value || '',
                    ...
                    producer,
                    composers,
                    composer1: _trim(f['Composer'] || ''),
                    composer2: _trim(f['Composer 2'] || ''),
                    composer3: _trim(f['Composer 3'] || ''),
                    aiBpm:    _trim(f['AI_BPM']    || ''),
                    aiKey:    _trim(f['AI_Key']    || ''),
                    aiMood:   _trim(f['AI_Mood']   || ''),
                    aiEnergy: _trim(f['AI_Energy'] || ''),
                    leadSilence: parseFloat(f['AI_LeadSilence']) || 0
                  };
                });
                fullAlbum = { ...album, tracks: transformedTracks };
                // Back-fill trackArtist from pre-loaded album.tracks if the API returned it empty.
                // album.tracks comes from groupAlbums() which checks both 'Track Artist' and
                // 'Tape Files::Track Artist', so it's a reliable source.
                const _origTracks = album.tracks || [];
                if (_origTracks.length > 0) {
                  fullAlbum.tracks.forEach((t, i) => {
                    if (!t.trackArtist) {
                      const orig = _origTracks.find(ot => ot.name === t.name) || _origTracks[i] || null;
                      if (orig && orig.trackArtist) t.trackArtist = orig.trackArtist;
                    }
                  });
                }
              }
      ...
      validTracks.forEach((t, idx) => {
        ...
        // Audio Lab button — always show it, use same candidateSrc as the play button
        const audioLabSrc = candidateSrc;
        const btnLab = document.createElement('button');
        btnLab.type = 'button';
        btnLab.className = 'btn small btn-audio-lab';
        btnLab.title = 'Open in Audio Lab';
        btnLab.innerHTML = '🎛️ Audio Lab';
        btnLab.addEventListener('click', () => {
          const trackTitle = encodeURIComponent(t.name || fullAlbum.title || 'track');
          const trackUrl   = encodeURIComponent(audioLabSrc);
          window.open(`/audio-lab?url=${trackUrl}&title=${trackTitle}`, '_blank');
        });
        controls.appendChild(btnLab);
    ...
    function renderAlbumPage(){
      ...
      if (currentMode === 'explore' || currentMode === 'landing' || currentMode === 'featured' || currentMode === 'genre') {
        // Skip frontend validation for explore/genre mode - backend already filtered for valid audio + artwork
      ...
      // Genre mode uses Load More (like explore) — pager prev/next use albumPage which conflicts with genreCurrentPage
      const shouldShowShuffle = currentMode === 'explore' || currentMode === 'landing' || currentMode === 'songs' || currentMode === 'featured';
      const shouldHidePager = shouldShowShuffle || currentMode === 'genre';
      if (shouldHidePager) {
        pagerEl.hidden = true;
        if (shuffleBtn) {
          // Only show shuffle in explore/landing/songs, not in genre mode
          shuffleBtn.hidden = currentMode === 'genre';
        }
      } else {
      ...
      // All modes: show ALL loaded albums (horizontal scroll — no pagination)
      pageAlbums = displayAlbums;
      ...
      pageAlbums.forEach(album => {
        ...
        const triggerSearch = () => {
          // Always open the tracks modal when clicking the album cover
        ...
        // Album icon — bottom-right of cover, opens tracks modal
        const albumIconBtn = document.createElement('button');
        albumIconBtn.className = 'card-album-icon-btn';
        albumIconBtn.title = 'Open album';
        albumIconBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg>';
        albumIconBtn.addEventListener('click', (e) => { e.stopPropagation(); openTracksModal(album, card); });
        wrap.appendChild(albumIconBtn);
    ...
    function handlePlay(btn, row, src){
      ...
      currentSrc = src;
      // Claim the unified queue (window.MADQueue, app.html): every play surface
      // registers so the player-bar ⏭/⏮ always advance the list the user is
      // actually playing. currentRow-based steppers skip unplayable rows.
      // autoEnded stays off — this engine has its own ended-advance chain.
      window.MADQueue = {
        autoEnded: false,
        next: function () { if (typeof window.madPlayNext === 'function') window.madPlayNext(); },
        prev: function () { if (typeof window.madPlayPrev === 'function') window.madPlayPrev(); }
      };
      ...
      // _PLAYER is the single owner of the audio element —
      // it sets player.src, calls player.play(), and updates the unified bar.
      const _m = row && row._meta ? row._meta : {};
      const playPromise = window._PLAYER.playTrack(src, {
        title:    _m.trackName   || '',
        artist:   _m.trackArtist || _m.albumArtist || '',
        artUrl:   _m.picture     || '',
        recordId: _m.trackRecordId || _m.recordId || ''
      });
    ...
    // Player-bar skip controls (app.html #upSkipNext/#upSkipPrev) live outside
    // this IIFE. They used to read window.currentRow — which is NEVER set — so
    // they silently no-opped. Bridge the real list-advance logic to the window;
    // it operates on the LIVE currentRow held in this closure. Shuffle/radio
    // queues are handled separately by window.MADPlayer.next/prev (player.js).
    window.madPlayNext = function () {
      if (randomSongsAutoplay.active && currentMode === 'songs' &&
          advanceRandomSongsAutoplayFrom(currentRow)) return true;
      return playNextPlayableFrom(currentRow);
    };
    window.madPlayPrev = function () {
      if (!currentRow) return false;
      let cand = currentRow.previousElementSibling;
      while (cand && (!cand._src || !cand._btn || cand._btn.disabled)) {
        cand = cand.previousElementSibling;
      }
      if (cand && cand._btn && cand._src) { handlePlay(cand._btn, cand, cand._src); return true; }
      return false;
    };
    // player.js (grid/discovery plays) calls this so a single-card play doesn't
    // leave a stale currentRow that would make ⏭ advance the wrong list.
    window.madClearListContext = function () { currentRow = null; currentBtn = null; };
    ...
    player.addEventListener('timeupdate', () => {
      updateProgressUI();
      if (player.paused) return; // never report progress when not actually playing
      const now = Date.now();
    ...
    player.addEventListener('pause', () => {
      if (!player.ended) {
        sendStreamEvent('STOPPED', getCurrentTrackMeta(), player.currentTime, player.duration);
      }
      ...
      if (currentRow && currentSrc === player.src) setNowPlayingFromRow(currentRow, false);
    ...
    player.addEventListener('play', () => {
      ...
      // Only sync nowPlayingInfo when the home-view track is what's playing.
      // If an external track (library / album / playlist) was started via _PLAYER,
      // player.src won't match currentSrc — don't overwrite with stale meta.
      if (currentRow && currentSrc === player.src) {
        setNowPlayingFromRow(currentRow, true);
      } else if (currentSrc !== player.src) {
        // Clear stale meta so the toggle handler falls back to direct play/pause.
        nowPlayingInfo.meta = null;
        nowPlayingInfo.isPlaying = false;
      }
      updateRandomSongsControlState();
    ...
    async function runExplore(startYear, options = {}){
      ...
      // Decade + genre combine: the genre dropdown ANDs into the explore
      // query (/api/explore?genre=…). Read live on every call so load-more
      // and refresh keep the filter without extra plumbing.
      const exploreGenre = (document.getElementById('searchGenre')?.value || '').trim();
      const exploreLabel = `${startYear}s${exploreGenre ? ' ' + exploreGenre : ''}`;
      ...
      if (isLoadMore) {
        showBusy(`Loading more ${exploreLabel}…`);
      } else {
        showBusy(`Loading ${exploreLabel}…`);
      ...
      if (exploreGenre) params.set('genre', exploreGenre);
      ...
      try {
        ...
        if (!j || !Array.isArray(j.items) || j.items.length === 0){
          if (!isLoadMore) {
            errorEl.hidden = false;
            errorEl.textContent = `No albums found for ${exploreGenre ? exploreGenre + ' in ' : ''}the ${start}s.`;
            rawItems = [];
            rawTotalFound = 0;
            rawNextOffset = 0;
            albumGroups = [];
        ...
        if (!albumGroups.length && !isLoadMore) {
          errorEl.hidden = false;
          errorEl.textContent = `No albums with complete audio and artwork found for ${exploreGenre ? exploreGenre + ' in ' : ''}the ${start}s.`;
          rawItems = [];
          rawTotalFound = 0;
          refreshPublicPlaylists([]);
          renderAlbumPage();
    ...
    async function runGenreExplore(genre, options = {}) {
      ...
      params.set('limit', '500'); // Max FM allows — maximises album variety per call
    ...
    if (searchDecadeDropdown) {
      searchDecadeDropdown.addEventListener('change', function() {
        ...
        if (!selectedValue) {
          // Decade cleared: fall back to a plain genre browse if a genre is
          // still selected, otherwise leave the current view alone.
          const genreLeft = (document.getElementById('searchGenre')?.value || '').trim();
          if (genreLeft) runGenreExplore(genreLeft);
          return;
        }
        const match = selectedValue.match(/^(\d{4})s$/);
        if (match) {
          const startYear = parseInt(match[1], 10);
          // runExplore reads the genre dropdown itself, so a selected genre
          // rides along automatically (decade + genre combined).
          runExplore(startYear);
        }
      });
    }
    ...
    if (exploreGenresEl){
      // Genres come from /api/genres — distinct values scanned from ACTUAL
      // catalogue records — so every button leads to real music. The old
      // hardcoded ~170-entry list included genres with zero records (clicking
      // one showed an empty page) and typos ("80'"); it survives only as the
      // offline fallback below.
      const paintGenres = (genres) => {
      ...
      fetch('/api/genres')
        .then(r => r.json())
        .then(data => {
          if (Array.isArray(data.genres) && data.genres.length) paintGenres(data.genres);
          else throw new Error('empty genre list');
        })
        .catch(() => {
          paintGenres(['African','African Jazz','Afrikaans','Afro Pop','Afro Soul','Amapiano','Blues','Bubblegum','Choral','Classical','Country','Dance','Disco','Folk','Funk','Gospel','Gqom','Hip Hop','House','Jazz','Kwaito','Kwela','Marabi','Maskandi','Mbhaqanga','Pop','Reggae','RnB','Rock','Soul','Traditional','World Music']);
        });
    }
    ...
    function updateLoadMoreButton(hasMore, currentCount, total) {
      ...
      // The rail renders ALL loaded albums, so there are no "pages" — the
      // button's only job is fetching more from the server. Show it only
      // while the server has more, and report a plain running count.
      if (!hasMore) {
        container.hidden = true;
        return;
      }
      ...
      info.textContent = `${currentCount.toLocaleString()} albums loaded`;
    }
    ...
    if (loadMoreBtn) {
      loadMoreBtn.addEventListener('click', async () => {
        if (currentMode === 'genre') {
          ...
          // The renderer shows ALL loaded albums, so a click must always grow
          // the loaded set — fetch until at least one page-worth of NEW albums
          // arrives (not until a page-counter target that's already surpassed,
          // which made the button a visible no-op).
          const targetCount = albumGroups.length + ALBUMS_PER_PAGE;
          while (albumGroups.length < targetCount && genreHasMore) {
            const beforeCount = albumGroups.length;
          ...
          // Only advance the page if albums exist for it — prevents empty-page fallback to page 1
          if (albumGroups.length > nextPageStart) {
            genreCurrentPage++;
          }
          renderAlbumPage();
          // Pass raw genreHasMore — updateLoadMoreButton calculates hasMorePages internally
          updateLoadMoreButton(genreHasMore, albumGroups.length, genreTotal);
        } else {
          // Decade explore mode pagination
          if (!currentExploreDecade) return;

          // Same as genre mode: the renderer shows everything loaded, so each
          // click must fetch at least one page-worth of NEW albums.
          const targetCount = albumGroups.length + ALBUMS_PER_PAGE;
          while (albumGroups.length < targetCount && exploreHasMore) {
            const beforeCount = albumGroups.length;
    ...
    if (goEl) {
      goEl.addEventListener('click', () => {
        ...
        // — unless a decade is also picked, in which case runExplore combines the two.
        if (!q && !artist && !album && !track && genre) {
          const decadeSel = (document.getElementById('searchDecade')?.value || '').trim().match(/^(\d{4})s$/);
          if (decadeSel) return runExplore(parseInt(decadeSel[1], 10));
    ...
    if (clearEl) {
      clearEl.addEventListener('click', () => {
        ...
        // Also reset the Genre/Decades toolbar buttons (they light up in the
        // accent colour while active — Clear must visibly clear them too)
        const genreSel = document.getElementById('searchGenre');
        const decadeSel = document.getElementById('searchDecade');
        if (genreSel) genreSel.value = '';
        if (decadeSel) decadeSel.value = '';
        lastQ = '';
        currentSearchFilters = normalizeSearchFilters();

        // Clear/Home must NOT stop playback (Spotify-style: browse & search
        // while the music keeps going). The now-playing bar persists; the
        // featured-grid re-render below replaces the old result cards.
        // (Previously paused the player + cleared now-playing here.)
    ...
    window.addEventListener('popstate', (e) => {
      // MADRouter (app.html) owns history entries it marks with state.mad. Only act
      // on share/foreign entries here so the two popstate handlers don't double-fire.
      if (e && e.state && e.state.mad) return;
...
window.loadFeaturedAlbums = loadFeaturedAlbums;
window.openTracksModal = openTracksModal;
// Explore engines — app.html's filter listeners route dropdown changes here.
// runGenreExplore was referenced by app.html but never exported, which made
// the genre dropdown a silent no-op (console.error branch). Fixed 2026-07-22.
window.runExplore = runExplore;
window.runGenreExplore = runGenreExplore;

// Direct album open: search then immediately open tracklist, skipping the intermediate results step
window.openAlbumDirect = async function(albumName, artistName) {
  if (!albumName) return;
  try {
    const params = new URLSearchParams();
    if (artistName) { params.set('artist', artistName); } else { params.set('q', albumName); }
    ...
    // Pick the best match (exact album title match preferred)
    const needle = albumName.toLowerCase();
    const match = groups.find(g => (g.title || '').toLowerCase() === needle) || groups[0];
    openTracksModal(match, null);
  } catch(e) { console.error('[openAlbumDirect]', e); }
};
...
// The choices come from /data/playlist-icons.json (served no-cache) rather than
// a list in here, so adding an icon is a file upload plus a line of JSON.
// We STORE the master url and DISPLAY the _300 derivative: ~15 KB a tile
// instead of the 300 KB–2.5 MB master, while keeping a bigger size reachable.

let playlistIconsPromise = null;

function playlistIconThumb(masterUrl, size) {
...
window.playlistIconThumb = playlistIconThumb;

function loadPlaylistIcons() {
  if (!playlistIconsPromise) {
    playlistIconsPromise = fetch('/data/playlist-icons.json')
  ...
  return playlistIconsPromise;
}

/**
 * Builds the selectable icon grid. `onChange` receives the selected master url
 * ('' when nothing is selected). Tiles whose image is missing remove
 * themselves, so a half-uploaded set still looks deliberate rather than broken.
 */
function buildPlaylistIconGrid(icons, initialUrl, onChange) {
  const grid = document.createElement('div');
  grid.style.cssText = 'display:grid;grid-template-columns:repeat(4,1fr);gap:8px;';

  let selected = initialUrl || '';
  let live = 0;

  const empty = document.createElement('p');
  empty.textContent = 'No icons available yet.';
  empty.style.cssText = 'margin:0;font-size:13px;color:var(--muted,#888);';
  empty.hidden = true;
  ...
  const paint = () => {
    grid.querySelectorAll('button').forEach((b) => {
      ...
      b.style.borderColor = on ? 'var(--accent,#8b5cf6)' : 'transparent';
  ...
  icons.forEach(({ name, url }) => {
    ...
    cell.style.cssText = 'position:relative;aspect-ratio:1;border-radius:8px;overflow:hidden;border:2px solid transparent;cursor:pointer;padding:0;background:#111;transition:border-color .15s,transform .15s;';
    ...
    img.onerror = () => {
      ...
      if (selected === url) { selected = ''; onChange(selected); }
    };
    img.onload = () => { live++; empty.hidden = true; };

    const lbl = document.createElement('span');
    lbl.textContent = name;
    lbl.style.cssText = 'position:absolute;bottom:0;left:0;right:0;font-size:9px;text-align:center;padding:2px 0 3px;background:rgba(0,0,0,0.6);color:#fff;font-weight:600;letter-spacing:.03em;text-transform:uppercase;';
...
window.massCreatePlaylist = async function() {
  ...
  const modal = document.createElement('div');
  modal.style.cssText = 'background:var(--surface,#1e1e2e);border-radius:16px;padding:28px 24px;width:min(480px,90vw);max-height:85vh;overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,0.6);display:flex;flex-direction:column;gap:20px;';

  const title = document.createElement('h2');
  title.textContent = 'New Playlist';
  title.style.cssText = 'margin:0;font-size:18px;font-weight:600;color:var(--text,#fff);';
  ...
  nameLabelText.textContent = 'Playlist name';
  nameLabelText.style.cssText = 'font-size:12px;color:var(--muted,#888);text-transform:uppercase;letter-spacing:.05em;';
  const nameInput = document.createElement('input');
  nameInput.type = 'text';
  nameInput.placeholder = 'e.g. Sunday Morning Jazz…';
  nameInput.maxLength = 80;
  nameInput.style.cssText = 'background:var(--surface2,#2a2a3e);border:1px solid var(--border,rgba(255,255,255,0.1));border-radius:8px;padding:10px 14px;color:var(--text,#fff);font-size:14px;outline:none;';
  nameLabel.append(nameLabelText, nameInput);

  const artLabel = document.createElement('div');
  artLabel.style.cssText = 'display:flex;flex-direction:column;gap:10px;';
  const artLabelText = document.createElement('span');
  artLabelText.textContent = 'Choose artwork (optional)';
  artLabelText.style.cssText = 'font-size:12px;color:var(--muted,#888);text-transform:uppercase;letter-spacing:.05em;';
  let selectedArtwork = '';
  const grid = buildPlaylistIconGrid(ICONS, '', (url) => { selectedArtwork = url; });

  artLabel.append(artLabelText, grid);
  ...
  cancelBtn.textContent = 'Cancel';
  cancelBtn.style.cssText = 'padding:9px 20px;border-radius:8px;border:1px solid var(--border,rgba(255,255,255,0.15));background:transparent;color:var(--muted,#aaa);cursor:pointer;font-size:13px;';
  const createBtn = document.createElement('button');
  createBtn.type = 'button';
  createBtn.textContent = 'Create';
  createBtn.style.cssText = 'padding:9px 22px;border-radius:8px;border:none;background:var(--accent,#8b5cf6);color:#fff;cursor:pointer;font-size:13px;font-weight:600;';
  btnRow.append(cancelBtn, createBtn);

  modal.append(title, nameLabel, artLabel, btnRow);
  overlay.appendChild(modal);
  document.body.appendChild(overlay);
  nameInput.focus();
  ...
  createBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/api/playlists', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, artwork: selectedArtwork || undefined })
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok && res.status !== 409) throw new Error(json?.error || 'Failed to create playlist');
      const playlist = json.playlist;
      if (playlist?.id && typeof setActivePlaylist === 'function') setActivePlaylist(playlist.id);
      cleanup();
    } catch (err) {
      window.alert(err?.message || 'Unable to create playlist');
  ...
  nameInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') createBtn.click(); if (e.key === 'Escape') cleanup(); });
};

/**
 * Icon picker for an EXISTING playlist. Resolves to the chosen master url, ''
 * to clear the icon, or null if the user backed out. The caller owns the save
 * so the request and the list refresh happen in the panel's own scope.
 */
window.massPickPlaylistIcon = async function(currentArtwork) {
  const ICONS = await loadPlaylistIcons();
  ...
  return new Promise((resolve) => {
    ...
    modal.setAttribute('aria-label', 'Choose playlist artwork');
    modal.style.cssText = 'background:var(--surface,#1e1e2e);border-radius:16px;padding:28px 24px;width:min(480px,90vw);max-height:85vh;overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,0.6);display:flex;flex-direction:column;gap:20px;';

    const title = document.createElement('h2');
    title.textContent = 'Playlist artwork';
    title.style.cssText = 'margin:0;font-size:18px;font-weight:600;color:var(--text,#fff);';

    const hint = document.createElement('p');
    hint.textContent = 'Tap an icon to choose it, or tap it again to remove it.';
    hint.style.cssText = 'margin:0;font-size:12px;color:var(--muted,#888);';

    const grid = buildPlaylistIconGrid(ICONS, selected, (url) => { selected = url; });
    ...
    saveBtn.style.cssText = 'padding:9px 22px;border-radius:8px;border:none;background:var(--accent,#8b5cf6);color:#fff;cursor:pointer;font-size:13px;font-weight:600;';
    btnRow.append(cancelBtn, saveBtn);

    modal.append(title, hint, grid, btnRow);
    overlay.appendChild(modal);
    document.body.appendChild(overlay);
    saveBtn.focus();

    cancelBtn.addEventListener('click', () => finish(null));
    saveBtn.addEventListener('click', () => finish(selected));
    overlay.addEventListener('click', (e) => { if (e.target === overlay) finish(null); });
    overlay.addEventListener('keydown', (e) => { if (e.key === 'Escape') finish(null); });
  });
};

// Public API: register a track started outside of handlePlay (e.g. album detail view).
// Must be called AFTER player.src is set but BEFORE player.play(), so that when the
// native 'play' event fires, nowPlayingInfo.meta already has the correct trackRecordId.
window.massSetCurrentTrack = function({ trackRecordId = '', trackName = '', trackArtist = '',
                                        albumTitle = '', picture = '' } = {}) {
  const p = document.getElementById('player');
  const src = (p && p.src && p.src !== window.location.href) ? p.src : '';
  if (!src) {
    console.warn('[massSetCurrentTrack] called before player.src is set');
    return;
  }
  nowPlayingInfo.meta = { trackRecordId, trackName, trackArtist, albumTitle, picture, src };
