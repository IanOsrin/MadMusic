...
// Models this process is willing to load. `index_info.model` is read out of a
// SQLite file downloaded over the network (SEMANTIC_DB_URL), and transformers.js
// resolves an unknown name by FETCHING it from the Hugging Face Hub and handing
// the resulting .onnx to onnxruntime-node, a native runtime. So the contents of
// that downloaded file would otherwise decide what code this service pulls in
// and executes. Allowlist it. Add a name here deliberately when the index is
// rebuilt against a different model — never widen this to accept whatever the
// file says. (2026-07-31)
const ALLOWED_EMBED_MODELS = new Set(
  (process.env.SEMANTIC_ALLOWED_MODELS || 'Xenova/multilingual-e5-small')
...
function load() {
  ...
  _loading = (async () => {
    try {
      ...
      if (!ALLOWED_EMBED_MODELS.has(info.model)) {
        // Fail closed: the whole feature degrades to unavailable, which callers
        // already handle, rather than loading whatever the downloaded index names.
        console.error(
          `[semantic-shelves] refusing to load unexpected embed model "${info.model}" — ` +
          `allowed: ${[...ALLOWED_EMBED_MODELS].join(', ')}. Shelves disabled.`
        );
        _ready = false;
        return false;
      }
...
// ── raw accessors for Maddie's query-understanding layer (maddie-lite) ──────

let _allMeta = null;
/**
 * Every track's parsed metadata: [{recordId, m}] where m is the raw meta
 * object (track/artist/album/year/genre/localGenre/mood/energy/bpm/language/
 * artworkUrl). Parsed once and cached (~62k rows, well under 100 MB).
 * Returns null when the index is unavailable.
 */
export async function getAllMeta() {
  if (_allMeta) return _allMeta;
  const ctx = await load();
  if (!ctx) return null;
  _allMeta = ctx.db.prepare('SELECT recordId, meta FROM tracks').all()
    .map((r) => ({ recordId: String(r.recordId), m: JSON.parse(r.meta) }));
  return _allMeta;
}

/**
 * KNN over the index returning raw parsed meta + distance:
 * [{recordId, m, distance}], or null when unavailable.
 */
export async function knnRaw(text, k = 100) {
  const ctx = await load();
  if (!ctx) return null;
  ...
  const rows = ctx.db.prepare(`
    ...
    ORDER BY v.distance
  `).all(qvec, k);
  return rows.map((r) => ({ recordId: String(r.recordId), m: JSON.parse(r.meta), distance: r.distance }));
}
...
export async function searchShelves(query, k = 10) {
  ...
  return rows.map((r) => {
    ...
    // The June index baked in some broken no-filename placeholders
    // (…/artwork/.jpg) — blank them so cards render the neutral sleeve
    // instead of a 403 image. Serve the 300px derivative when the thumbs
    // flag is on (same rule as the API item builders).
    let artworkUrl = m.artworkUrl || '';
    if (isBrokenArtworkUrl(artworkUrl)) artworkUrl = '';
    if (artworkUrl && process.env.ARTWORK_THUMBS === 'true') {
      artworkUrl = thumbArtworkUrl(artworkUrl, 300);
    }
    return {
