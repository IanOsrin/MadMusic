...
import { fmCreateRecord, fmFindRecords, fmUpdateRecord } from '../fm-client.js';
 ...
 * Used to enforce one-trial-per-email. Matches on the canonical mailbox
 * (dots/plus-aliases collapsed), so `ian+2@gmail.com` can't re-claim.
 */
export async function findTrialTokenByEmail(email) {
  ...
  return tokenData.tokens.find(
    t => t.type === 'trial' && t.email && canonicalizeEmail(t.email) === canon
  ) || null;
}

/**
 * Durable one-trial-per-email check against FileMaker. The JSON file lives on
 * the web service's disk and does NOT survive every deploy — FM is the only
 * memory that does, which is exactly how the same email kept re-claiming
 * trials (2026-08-27). Returns the FM record, null when clean, and THROWS on
 * FM errors so the caller can decide how to degrade.
 */
export async function findTrialTokenInFM(email) {
  if (!email) return null;
  const layout     = process.env.FM_TOKENS_LAYOUT || 'API_Access_Tokens';
  const normalised = email.trim().toLowerCase();
  const canon      = canonicalizeEmail(email);
  const queries = [{ 'Email': fmExactMatch(normalised), 'Token_Type': fmExactMatch('trial'), 'Active': '1' }];
  if (canon !== normalised) {
    queries.push({ 'Email': fmExactMatch(canon), 'Token_Type': fmExactMatch('trial'), 'Active': '1' });
  }
  const result = await fmFindRecords(layout, queries, { limit: 1 });
  if (result.ok) return result.data.length ? result.data[0] : null;
  if (String(result.code) === '401') return null; // FM 401 = "no records match"
  throw new Error(`FM trial lookup failed: ${result.msg || result.status}`);
}

/**
 * Undo a just-created token whose delivery failed: drop it from the JSON store
 * and deactivate the FM record (Active=0 keeps an audit trail, and the trial
 * dedupe only counts Active=1, so the email can retry cleanly).
 */
export async function revokeToken(code, reason) {
  if (!code) return;
  const tokenData = await loadAccessTokens();
  const before    = tokenData.tokens.length;
  tokenData.tokens = tokenData.tokens.filter(t => t.code !== code);
  if (tokenData.tokens.length !== before) await saveAccessTokens(tokenData);

  try {
    const layout = process.env.FM_TOKENS_LAYOUT || 'API_Access_Tokens';
    const result = await fmFindRecords(layout, [{ 'Token_Code': fmExactMatch(code) }], { limit: 1 });
    if (result?.data?.length) {
      await fmUpdateRecord(layout, result.data[0].recordId, {
        'Active': 0,
        'Notes':  `${result.data[0].fieldData?.Notes || ''} [revoked: ${reason || 'delivery failed'}]`.trim()
      });
    }
  } catch (err) {
    console.error(`[MASS] revokeToken: FM deactivate failed for ${code}:`, err?.message || err);
  }
  console.log(`[MASS] Token ${code} revoked (${reason || 'delivery failed'})`);
}

// ── Free trial: straight on, confirm by email (Ian, 2026-09-26) ─────────────────
// A trial starts as a 1-day code handed straight to the browser, so the listener is in at
// once. The trial email's "Confirm" link extends that code to the full trial length. An
// invented address therefore gets a day, not a week — the reason the Aug 2026 abuse fix
// stopped returning the code at all.
export const TRIAL_UNCONFIRMED_DAYS = 1;

// FileMaker timestamps as the rest of the token code writes them ("M/D/YYYY H:MM:SS",
// server-local); parsed and re-written in the same local time, so a pure +N-days shift is
// independent of FM_TIMEZONE_OFFSET.
function shiftFmTimestamp(fmTs, days) {
  const t = new Date(String(fmTs || '').trim());
  if (Number.isNaN(t.getTime())) return null;
  t.setDate(t.getDate() + days);
  return `${t.getMonth() + 1}/${t.getDate()}/${t.getFullYear()} ${t.getHours()}:${String(t.getMinutes()).padStart(2, '0')}:${String(t.getSeconds()).padStart(2, '0')}`;
}

/**
 * Extend an unconfirmed trial code to `fullDays`. Idempotent: a second click reports
 * alreadyConfirmed. Works before first use (only the duration changes — first use stamps
 * the expiry from it) and after (the stamped Expiration_Date moves out too).
 * Returns { ok, alreadyConfirmed?, reason? }.
 */
export async function confirmTrialToken(code, fullDays) {
  const fullSeconds = fullDays * 24 * 60 * 60;
  const extraDays   = fullDays - TRIAL_UNCONFIRMED_DAYS;
  const layout = process.env.FM_TOKENS_LAYOUT || 'API_Access_Tokens';
  const found  = await fmFindRecords(layout, [{ 'Token_Code': fmExactMatch(code) }], { limit: 1 });
  const rec    = found?.data?.[0];
  if (!rec) return { ok: false, reason: 'not found' };
  const f = rec.fieldData || {};
  if (String(f.Token_Type) !== 'trial') return { ok: false, reason: 'not a trial' };
  if (String(f.Active) !== '1')        return { ok: false, reason: 'inactive' };
  if (Number.parseInt(f.Token_Duration_Hours, 10) >= fullSeconds) return { ok: true, alreadyConfirmed: true };

  const update = {
    'Token_Duration_Hours': String(fullSeconds),   // seconds, despite the name
    'Notes': `${f.Notes || ''} [email confirmed ${new Date().toISOString().slice(0, 10)}]`.trim(),
  };
  if (f.First_Used && f.Expiration_Date) {
    const moved = shiftFmTimestamp(f.Expiration_Date, extraDays);
    if (moved) update['Expiration_Date'] = moved;
  }
  await fmUpdateRecord(layout, rec.recordId, update);

  // Keep the JSON fallback copy in step.
  const tokenData = await loadAccessTokens();
  const local = tokenData.tokens.find((t) => t.code === code);
  if (local?.expirationDate) {
    const e = new Date(local.expirationDate);
    e.setDate(e.getDate() + extraDays);
    local.expirationDate = e.toISOString();
    local.emailConfirmed = true;
    await saveAccessTokens(tokenData);
  }
  console.log(`[MASS] Trial ${code} confirmed by email → ${fullDays} days`);
  return { ok: true };
}
...
export async function createSubscriptionToken(subscriptionCode, planCode, email, billingDays = 31) {
  ...
  // createAccessToken has always refused to store the 'unknown' sentinel as an
  // address; this one did not, so a subscription paid for without an email on
  // the Paystack payload wrote the literal string into FM's Email field —
  // breaking the account binding and the free-trial dedupe that keys on it.
  const realEmail = (email && email !== 'unknown') ? String(email).trim() : null;
  const notes = `Paystack subscription (${realEmail || 'NO EMAIL ON PAYMENT'}, sub: ${subscriptionCode}, plan: ${planCode})`;
  ...
  if (realEmail) fmFields['Email'] = realEmail;
...
export async function renewSubscriptionToken(subscriptionCode, billingDays = 31) {
  ...
  try {
    ...
    const result = await fmFindRecords(layout, [{ 'Token_Code': fmExactMatch(token.code) }], { limit: 1 });
    if (result?.data?.length) {
