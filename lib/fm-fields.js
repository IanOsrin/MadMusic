import { trackIsEligible } from './track-eligibility.js';
/**
...
export const FM_HERO_LAYOUT          = process.env.FM_HERO_LAYOUT           || 'API_Hero_Featured';
export const HERO_TARGET_TYPES       = ['track', 'album', 'playlist', 'external'];
export const FM_ARTIST_BIO_LAYOUT    = process.env.FM_ARTIST_BIO_LAYOUT     || 'API_Artist_Bio';
export const FM_PLAYLIST_ART_LAYOUT  = process.env.FM_PLAYLIST_ART_LAYOUT   || 'API_Playlist_Art';
...
// Cache the expensive {value, field} map per fields object. WeakMap lets the
// entries be garbage-collected when the fields object is dropped, so holding
// the reference is safe across thousands of records.
const fieldInfoMapCache = new WeakMap();

function getFieldInfoMap(fields) {
  if (fieldInfoMapCache.has(fields)) return fieldInfoMapCache.get(fields);
  const map = new Map();
  for (const [key, raw] of Object.entries(fields)) {
    if (raw === null || raw === undefined) continue;
    const str = rawToTrimmedString(raw);
    if (!str) continue;
    if (!map.has(key)) map.set(key, { value: str, field: key });
    const normalized = normalizeFieldKey(key);
    if (normalized && !map.has(normalized)) {
      map.set(normalized, { value: str, field: key });
    }
  }
  fieldInfoMapCache.set(fields, map);
  return map;
}

export function pickFieldValueCaseInsensitive(fields = {}, candidates = []) {
  const map = getFieldInfoMap(fields);
...
/**
 * Should this record appear in the app at all?
 *
 * Two rules now. The long-standing Visibility field, AND the client's
 * eligibility rule (ISRC + UPC + cover, 2026-09-02). Eligibility is folded in
 * HERE deliberately: every rail, the Postgres mirror, catalogue pages, trending
 * and the featured shelves already ask this one question, so one change covers
 * them all and no surface can be forgotten. Playlists do not call it and are
 * filtered separately — that is exactly where the gap was spotted.
 */
export function recordIsVisible(fields = {}) {
  if (!trackIsEligible(fields)) return false;
