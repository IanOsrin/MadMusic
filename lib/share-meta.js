...
export async function getTrackShareMeta(recordId) {
  ...
  return {
    ...
    // Squeeze stray spaces: one album is filed as "XPD  2278" (a typo), and the
    // app's /api/album?cat= lookup matches the value exactly — so a shared link
    // to it opened the home screen instead of the album (2026-09-16).
    catalogue: firstField(f, CATALOGUE_FIELD_CANDIDATES).replace(/\s+/g, ' ').trim(),
    artworkUrl
  };
}
