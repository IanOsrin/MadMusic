...
async function maybeDownload() {
  ...
  if (!url) return;
  log.info(`suggest.db missing — downloading from ${url}`);
  ...
  fs.renameSync(tmp, DB_PATH); // atomic swap so a half-written file is never opened
  log.info(`downloaded suggest.db (${(buf.length / 1024 / 1024).toFixed(1)} MB)`);
}
 ...
 * index in the not-ready state and are logged.
 */
export async function initSemanticIndex() {
  ...
  try {
    ...
    // Lazy-load the native modules — a load failure here disables the feature
    // (caught below) rather than crashing app boot.
    const { default: Database } = await import('better-sqlite3');
    const sqliteVec = await import('sqlite-vec');
    const handle = new Database(DB_PATH, { readonly: true, fileMustExist: true });
    sqliteVec.load(handle);

    info = Object.fromEntries(handle.prepare('SELECT key, value FROM index_info').all().map((r) => [r.key, r.value]));
    if (Number(info.dim) !== VEC_DIM) {
      log.error(`index dim ${info.dim} != expected ${VEC_DIM} — refusing to load`);
      handle.close();
      return false;
    }
