...
export function suggestAlbums(seed = {}, n = 10, opts = {}) {
  ...
  // "Fresh set each visit": take a relevance window of the closest candidates
  // and shuffle it, so re-opening surfaces a different — but still similar —
  // selection. Deterministic (closest-first) when shuffle is off.
  let ranked = candidates;
  if (shuffle) {
    const windowSize = Math.min(candidates.length, Math.max(limit * SHUFFLE_POOL_FACTOR, limit + 5));
    ranked = shuffled(candidates.slice(0, windowSize));
  }

  // Split into those WITH a usable cover and those without. A rail full of
  // placeholders reads as broken, so we fill from cover-having albums first and
  // only top up with art-less ones to reach `limit`. Order within each group is
  // preserved (distance order, or the shuffled window order).
  const withArt = ranked.filter((c) => c.artworkSrc);
  const without = ranked.filter((c) => !c.artworkSrc);
