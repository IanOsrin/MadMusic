...
// ── "Suggested for You": many seeds, one listener ───────────────────────────
// suggestAlbums() answers "albums like THIS album". A listener's rail needs
// "albums like everything THEY play": each album they have played is a seed,
// weighted by how much and how recently they played it (lib/listening-profile.js
// supplies the weights). Every seed's nearest albums are scored by similarity ×
// the seed's share of the listening, summed across seeds — so an album close to
// several things a listener loves outranks one close to a single album they
// played once. Albums they have already played are never suggested.
const LISTENER_MAX_SEEDS = Math.max(1, parseInt(process.env.PERSONAL_RAIL_MAX_SEEDS || '30', 10) || 30);
const LISTENER_NEIGHBOURS = Math.max(10, parseInt(process.env.PERSONAL_RAIL_NEIGHBOURS || '60', 10) || 60);
const LISTENER_MAX_PER_SEED = 3;
// Below this cosine similarity a neighbour is no longer "like" the seed: in this
// index half of all albums have neighbours at 0.94, and a children's album's
// nearest unrelated match (an amapiano record) sat under 0.9.
const LISTENER_MIN_SIMILARITY = 0.9;
// A bio-credited link counts like a neighbour this close — on par with the best
// semantic matches when the artist plays on the album, a little less when they
// only wrote a song on it (a cover can sit in a very different style).
const BIO_LINK_SIMILARITY = { features: 0.95, produced: 0.9, 'wrote for': 0.8, 'appears on': 0.75 };
const GENERIC_ALBUM = /^\s*singles?\b/i;
const COMPILATION_ARTIST = /^(various|various artists|va|gallo moods)$/;

// A personal rail should be albums by someone. Four or more credited acts
// ("U Muzikayafani No-Makhulana, Khula Special, Madlala Brothers, Thikazisa")
// is a compilation under another name.
function isCompilationArtist(name) {
  const n = norm(name);
  return COMPILATION_ARTIST.test(n) || n.split(/\s*,\s*/).length >= 4;
}

// Looser artist identity for the personal rail: the part before "en sy", "met",
// "and his …" and the like, reduced to its consonants, so the catalogue's
// spellings of one act count once (Oliver Mtukudzi / Oliver Mutukudzi,
// Niek Potgieter / Niek Potgieter En Sy Engelse Konsertina).
function listenerArtistKey(name, aliases) {
  const canonical = aliases?.get(name);
  if (canonical) name = canonical;
  const core = artistCoreKey(name)
    .split(/\s+(?:en|met|and|with|his|her|sy|the)\s+/)[0]
    .trim();
  const skeleton = core.replace(/[aeiouy\s'’.-]/g, '');
  return skeleton.length >= 3 ? skeleton : core;
}

/**
 * @param {{cat?:string,title?:string,artist?:string,weight:number}[]} played
 * @param {number} [n=10]
 * @returns {{ ok:boolean, items:object[], seedsUsed:number, seedsResolved:number }}
 */
export function suggestForListener(played = [], n = 10, opts = {}) {
  if (!ready || !db) return { ok: false, items: [], seedsUsed: 0, seedsResolved: 0 };
  const limit = Math.max(1, Math.min(50, Number(n) || 10));
  const maxPerArtist = Math.max(1, Number(opts.maxPerArtist) || DEFAULT_MAX_PER_ARTIST);
  // Compilations match a little of everyone's listening, so by default they are
  // left out of a personal rail altogether.
  const maxCompilations = Math.max(0, Number(opts.maxCompilations) || 0);

  // Resolve every played album to an index row; weights of variants that land
  // on the same album (e.g. two catalogue spellings) add up.
  const weights = new Map();
  for (const p of played) {
    const r = resolveSeed(p);
    if (!r || !(p.weight > 0)) continue;
    weights.set(r.key, (weights.get(r.key) || 0) + p.weight);
  }
  // Square-root the listening weight: one album played on repeat should lead
  // the rail, not become all of it (a first test gave one listener ten
  // suggestions traced to a single album).
  for (const [k, w] of weights) weights.set(k, Math.sqrt(w));
  const playedKeys = new Set(weights.keys());
  const seeds = [...weights.entries()].sort((a, b) => b[1] - a[1]).slice(0, LISTENER_MAX_SEEDS);
  const totalWeight = seeds.reduce((sum, [, w]) => sum + w, 0);
  if (!seeds.length || !(totalWeight > 0)) return { ok: true, items: [], seedsUsed: 0, seedsResolved: playedKeys.size };

  const vecStmt = db.prepare('SELECT embedding FROM vec_albums WHERE rowid = ?');
  ...
  // Bios (opts.bio, from routes/artist-bio.js getArtistBioLinks): aliases fold
  // an act's spellings into one identity, and each played artist's "Related
  // Music" albums score as if they were the closest possible neighbour.
  const aliases = new Map();
  const relatedByArtist = new Map();
  for (const a of opts.bio?.artists || []) {
    const keys = [a.name, ...(a.aliases || [])].map((k) => artistCoreKey(k)).filter(Boolean);
    for (const k of keys) {
      if (!aliases.has(k)) aliases.set(k, a.name);
      if (a.related?.length && !relatedByArtist.has(k)) relatedByArtist.set(k, a);
    }
  }
  const aliasLookup = { get: (n) => aliases.get(artistCoreKey(n)) };

  const scored = new Map();   // albumKey → { score, meta, bestSeed, bestContribution, link }
  const bioArtistShare = new Map();  // bio artist name → { share, seedKey }
  for (const [seedKey, w] of seeds) {
    const artist = byKey.get(seedKey)?.meta?.artist;
    const bioArtist = artist && relatedByArtist.get(artistCoreKey(aliasLookup.get(artist) || artist));
    if (!bioArtist) continue;
    const cur = bioArtistShare.get(bioArtist.name) || { share: 0, seedKey, bio: bioArtist };
    cur.share += w / totalWeight;
    bioArtistShare.set(bioArtist.name, cur);
  }
  for (const { share, seedKey, bio } of bioArtistShare.values()) {
    for (const rel of bio.related) {
      const r = resolveSeed({ title: rel.title, artist: rel.artist });
      if (!r || playedKeys.has(r.key)) continue;
      const entry = scored.get(r.key) || { score: 0, meta: JSON.stringify(r.row.meta), bestSeed: null, bestContribution: 0 };
      const contribution = share * (BIO_LINK_SIMILARITY[rel.relation] || 0.75);
      entry.score += contribution;
      if (contribution > entry.bestContribution) {
        entry.bestContribution = contribution;
        entry.bestSeed = seedKey;
        entry.link = `${bio.name} ${rel.relation} this`;
      }
      scored.set(r.key, entry);
    }
  }

  for (const [seedKey, w] of seeds) {
    const row = byKey.get(seedKey);
    const v = row && vecStmt.get(BigInt(row.id));
    if (!v) continue;
    const share = w / totalWeight;
    for (const h of knnStmt.all(v.embedding, LISTENER_NEIGHBOURS)) {
      if (playedKeys.has(h.albumKey)) continue;
      // Vectors are L2-normalised, so cosine similarity = 1 − d²/2.
      const sim = Math.max(0, 1 - (h.distance * h.distance) / 2);
      if (sim < LISTENER_MIN_SIMILARITY) continue;
      const contribution = share * sim;
      const entry = scored.get(h.albumKey) || { score: 0, meta: h.meta, bestSeed: null, bestContribution: 0 };
      entry.score += contribution;
      if (contribution > entry.bestContribution) { entry.bestContribution = contribution; entry.bestSeed = seedKey; entry.link = null; }
      scored.set(h.albumKey, entry);
    }
  }
  ...
  for (const entry of ranked) {
    ...
    const card = toCard(JSON.parse(entry.meta));
    if (!card.artworkSrc) continue;                    // a rail of placeholder sleeves reads as broken
    // Holding albums for loose singles ("singles - 9806-13") sit near the centre
    // of everything, so they would top every listener's rail without being an
    // album anyone chose.
    if (GENERIC_ALBUM.test(card.album)) continue;
    // At most three suggestions may trace back to the same played album, so the
    // rail reflects the breadth of someone's listening.
    const seedCount = perSeed.get(entry.bestSeed) || 0;
    if (seedCount >= LISTENER_MAX_PER_SEED) continue;
    const artistKey = isCompilationArtist(card.artist) ? 'various artists' : listenerArtistKey(card.artist, aliasLookup);
    if (artistKey === 'various artists') {
      if (compilations >= maxCompilations) continue;
      compilations += 1;
    } else if (artistKey) {
    ...
    perSeed.set(entry.bestSeed, seedCount + 1);
    const because = entry.bestSeed && byKey.get(entry.bestSeed)?.meta;
    items.push({ ...card, score: +entry.score.toFixed(4), because: because ? { album: because.album || '', artist: because.artist || '', ...(entry.link ? { link: entry.link } : {}) } : null });
  }
  return { ok: true, items, seedsUsed: seeds.length, seedsResolved: playedKeys.size };
}
