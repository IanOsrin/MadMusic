...
export function getClientIP(req) {
  // Prefer Express's req.ip: once `trust proxy` is configured as a hop count it
  // resolves the correct client IP from the right position in X-Forwarded-For
  // and cannot be spoofed by an extra left-most XFF entry from the client.
  if (typeof req?.ip === 'string' && req.ip) return req.ip;
  // Fall back to parsing X-Forwarded-For only when req.ip is absent (e.g. a raw
  // http server without Express). This is spoofable, hence the fallback only.
...
export const resolveRequestOrigin = (req) => {
  // Prefer a trusted, configured absolute base URL. When APP_URL/APP_BASE_URL is
  // set we IGNORE request headers entirely — otherwise an attacker can inject a
  // forged Host/X-Forwarded-Host header to make us mint share/callback links
  // pointing at a host they control (host-header injection).
  const configured = (process.env.APP_URL || process.env.APP_BASE_URL || '').trim();
  if (configured) return configured.replace(/\/+$/, '');
  // Dev / unconfigured fallback only: derive origin from request headers.
