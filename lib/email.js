...
export function sendTrialEmail(customerEmail, tokenCode, confirmUrl) {
  if (!emailTransporter) {
  ...
  // Straight on, confirm later (2026-09-26): the listener is already signed in on the device
  // they signed up on. This email's job is the Confirm button (1 day → 7 days) and the code
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto; padding: 24px;">
      ...
      <p style="color: #555; margin-bottom: 20px;">Welcome to MAD — Music Africa Direct! You're already signed in on the device you signed up on.</p>
      ${confirmUrl ? `
      <p style="color: #555; margin-bottom: 16px;"><strong>Confirm your email within 24 hours to keep all 7 days</strong> (otherwise the trial ends after 1 day):</p>
      <p style="text-align: center; margin: 0 0 28px;">${button(confirmUrl, 'Confirm my email')}</p>` : ''}
      <p style="color: #555; margin-bottom: 8px;">Listening on another phone or computer? Use this link, or enter the code:</p>
      <p style="text-align: center; margin: 0 0 14px;"><a href="${escapeHtml(listenUrl)}" style="color: #7c3aed; font-weight: bold;">Start listening</a></p>
      <div style="background: #f4f4f4; border-radius: 8px; padding: 18px; text-align: center; margin-bottom: 24px;">
        <span style="font-size: 26px; font-weight: bold; letter-spacing: 2px; color: #1a1a1a;">${escapeHtml(tokenCode)}</span>
      </div>
      <hr style="border: none; border-top: 1px solid #ddd; margin: 24px 0;" />
...
// Goes to the support inbox (CONTACT_TO, default serverdev@gmail.com) with Ian as a
...
export function sendContactEmail({ email, message, details }) {
  ...
  const to  = process.env.CONTACT_TO  || 'serverdev@gmail.com';
