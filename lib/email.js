...
export function sendTrialEmail(customerEmail, tokenCode) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto; padding: 24px;">
      ...
      <p style="color: #555; margin-bottom: 24px;">Enter this token on the Mass Music app to activate your 7-day free trial.</p>
      <hr style="border: none; border-top: 1px solid #ddd; margin: 24px 0;" />
  ...
  return emailTransporter.sendMail({
    ...
    throw err; // the token now travels ONLY by email — the route must know delivery failed
  });
}
