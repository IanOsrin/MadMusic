...
// True when the URL is the broken no-filename ingest placeholder. Exported for
// consumers that carry artwork URLs outside FM field objects (e.g. the
// semantic index, whose June build baked some of these in).
export function isBrokenArtworkUrl(url) {
  return typeof url === 'string' && REGEX_BROKEN_ARTWORK.test(url);
}
