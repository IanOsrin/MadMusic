...
export const STREAM_EVENT_TYPES     = new Set(['PLAY', 'PROGRESS', 'PAUSE', 'SEEK', 'END', 'ERROR', 'STOPPED']);
// Terminal = the listen is OVER; the session-record cache is cleared and a
// subsequent PLAY starts a NEW record. STOPPED is deliberately NOT terminal
// (audit F2, 2026-07-07): the desktop client fires STOPPED on EVERY pause, so
// treating it as terminal split one listen into a new FM record per
// pause/resume cycle — inflating play counts and fragmenting TotalPlayedSec.
// A pause is part of the same listen; only END and ERROR close a record.
...
export async function findStreamRecord(sessionId, trackRecordId) {
  ...
  const query = [{ SessionID: fmExactMatch(sessionId), TrackRecordID: fmExactMatch(trackRecordId) }];
  const sort  = [
