...
// ── User-chosen playlist artwork ─────────────────────────────────────────────
// The icon a user picks when creating or editing their own playlist. Stored as
// the MASTER url (artwork/<file>.jpg), same convention as album artwork, so a
// larger derivative stays reachable later; surfaces that render it small ask
// for the _300 derivative themselves via playlistThumbUrl().
//
// Anything not served from our own artwork prefix is rejected outright — the
// value is written to FM and rendered back into an <img> for every viewer of a
// shared playlist, so it must never become an arbitrary remote URL.

export const S3_ARTWORK_BASE =
  'https://mass-music-audio-files.s3.eu-north-1.amazonaws.com/artwork/';

/** Returns a safe artwork URL, or '' if the input isn't one of ours. */
export function normalizePlaylistArtwork(value) {
  if (typeof value !== 'string') return '';
  const url = value.trim();
  if (!url.startsWith(S3_ARTWORK_BASE)) return '';
  // Reject anything trying to climb back out of the prefix.
  if (url.includes('..')) return '';
  return url;
}

/**
 * Master artwork url -> pre-generated WebP derivative. Mirrors
 * thumbArtworkUrl() in lib/track.js; kept here because playlist artwork is
 * rewritten for display rather than by the ARTWORK_THUMBS response filter.
 */
export function playlistThumbUrl(url, size = 300) {
...
export const sanitizePlaylistForShare = (playlist) => {
  ...
  return {
    ...
    name: playlist.name || '',
    artwork: normalizePlaylistArtwork(playlist.artwork),
...
// ── FM-managed playlist art (API_Playlist_Art) ───────────────────────────────
// Optional override for the file-based covers above: an FM layout with
// Playlist_Name + Image_S3_URL lets editors change covers in FileMaker (point at
// a new S3 image) with no deploy. Keyed by the SAME slug the file resolver uses,
// so FM names tolerate minor case/spacing differences vs the PublicPlaylist value.

export function buildPlaylistArtMap(records) {
  const map = new Map();
  for (const r of records || []) {
    const f = r.fieldData || {};
    const slug = slugifyPlaylistName(String(f.Playlist_Name || '').trim());
    const url = String(f.Image_S3_URL || '').trim();
    // First record wins; only accept real http(s) URLs.
    if (slug && /^https?:\/\//i.test(url) && !map.has(slug)) map.set(slug, url);
  }
  return map;
}

/**
 * slug -> Category ('Scene' | 'Artist' | 'Decade' | 'Theme') from the same
 * API_Playlist_Art records. Category classifies the PLAYLIST, so it is what
 * decides which rail a playlist belongs on — kept in its own map so the art
 * map's value stays a plain URL for every existing caller.
 *
 * A record with a category but no image still counts: the category is set from
 * GalloIngest's Playlists tab, which upserts a record for playlists that have
 * no cover yet.
 */
export function buildPlaylistCategoryMap(records) {
  const map = new Map();
  for (const r of records || []) {
    const f = r.fieldData || {};
    const slug = slugifyPlaylistName(String(f.Playlist_Name || '').trim());
    const category = String(f.Category || '').trim();
    if (slug && category && !map.has(slug)) map.set(slug, category);
  }
  return map;
}

export function playlistCategoryLookup(catMap, name) {
  if (!catMap || !name) return '';
  return catMap.get(slugifyPlaylistName(name)) || '';
}

export function playlistArtLookup(artMap, name) {
  if (!artMap || !name) return null;
  return artMap.get(slugifyPlaylistName(name)) || null;
}
