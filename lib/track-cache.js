...
import { usePostgresMetadata } from './metadata-source.js';
import { FM_LAYOUT } from './fm-fields.js';
import { pgTrackById } from './catalog-store-pg.js';
...
export async function getTrackRecordCached(layout, recordId) {
  ...
  const p = (async () => {
    try {
      // Catalog-layout lookups come from the Postgres mirror when enabled; other
      // layouts (none today) always go to FileMaker.
      const record = (usePostgresMetadata() && layout === FM_LAYOUT)
        ? await pgTrackById(recordId)
        : await fmGetRecordById(layout, recordId);
