...
export const PAYSTACK_PLANS = {
  '1-day':  { amount: 250,   label: '1 Day Access',  days: 1,  display: 'R2.50'  },
  '7-day':  { amount: 750,   label: '7 Day Access',  days: 7,  display: 'R7.50'  },
  '30-day': { amount: 3999,  label: '30 Day Access', days: 30, display: 'R39.99' }
};
...
export async function paystackRequest(method, endpoint, body) {
  ...
  // Read the body ONCE as text so the raw text is still available for logging if
  // JSON parsing fails (the body stream can only be consumed once — calling
  // response.text() after response.json() would always fail on an empty stream).
  ...
  try {
    ...
    throw new Error(`Paystack API error: ${response.status} ${response.statusText}`, { cause: parseErr });
  }
  if (!response.ok) {
 ...
 *
 * Security-critical:
 *  - rawBody MUST be the unparsed Buffer (or string) of the request body.
 *    If express.json has already parsed it into an Object, HMAC will be
 *    computed over a re-serialised payload and verification will *appear*
 *    to fail safely (return false) — but if signature is somehow valid
 *    over a parsed shape, that's an exploit. We hard-reject anything that
 *    isn't a Buffer or a string.
 *  - Comparison uses timingSafeEqualStr so an attacker can't byte-by-byte
 *    guess the signature by measuring server response time.
 */
export function verifyPaystackWebhook(rawBody, signature) {
  ...
  if (!Buffer.isBuffer(rawBody) && typeof rawBody !== 'string') {
    // Guards against the classic foot-gun where express.json() parses the
    // body before the webhook handler sees it.
    console.warn('[Paystack] webhook rawBody is not a Buffer or string; refusing to verify');
    return false;
  }
