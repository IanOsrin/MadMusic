...
export async function pgNewReleases() {
  // The /new-releases rail keys off a "New_Release" field that is NOT on the
  // API_Album_Songs layout, so the FM path returns empty too — no mirror column
  // exists. Return empty for parity. See docs/postgres-mirror.md (open items).
  return [];
}
